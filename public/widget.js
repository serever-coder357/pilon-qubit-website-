(function () {
  ...
  console.log("[PQV Widget] Script file loaded");
  ...
  function initWidget() {
    try {
      console.log("[PQV Widget] initWidget called");
      ...
      try {
        ...
        safeLogError("[PQV Widget] scriptTag lookup error", err);
      }

      if (!scriptTag) {
        console.warn(
          "[PQV Widget] No script tag with data-widget-id found – using defaults"
        );
      }

      const WIDGET_ID =
        (scriptTag && scriptTag.getAttribute("data-widget-id")) ||
        "pilonqubit-main";
      const API_BASE =
        (scriptTag && scriptTag.getAttribute("data-api-url")) ||
        window.location.origin;

      console.log("[PQV Widget] WIDGET_ID:", WIDGET_ID);
      console.log("[PQV Widget] API_BASE:", API_BASE);
      ...
      function getVisitorId() {
        try {
          ...
          safeLogError("[PQV Widget] localStorage error", err);
          return (
            "visitor_fallback_" +
            Math.random().toString(36).substr(2, 9) +
            Date.now()
      ...
      console.log("[PQV Widget] VISITOR_ID:", VISITOR_ID);
      ...
      const widgetConfig = {
        ...
        welcomeMessage:
          "Hi! I'm your AI concierge at Pilon Qubit Ventures.",
      ...
      function createWidget() {
        console.log("[PQV Widget] createWidget called");
        ...
        <div id="chat-header">
          <div>
            ...
            <div id="chat-header-status">${
              widgetConfig.isOnline ? "Online" : "Offline"
            }</div>
          </div>
    ...
    `;
        ...
        console.log("[PQV Widget] Widget container appended to DOM");
        ...
        if (!button || !windowEl || !messagesEl || !input || !sendBtn) {
          console.error(
            "[PQV Widget] Missing core elements; widget will not render correctly"
          );
          return;
        }
        ...
        // Load existing conversation & welcome
        loadConversation();
        addWelcomeMessage();
      ...
      `;
        ...
        function setupVoiceRecording(voiceBtn) {
          if (
            ...
            console.warn(
              "[PQV Widget] Voice recording not supported in this browser"
            );
            return;
          }
          ...
          voiceBtn.addEventListener("click", async () => {
            try {
              if (!mediaRecorder || mediaRecorder.state === "inactive") {
                console.log("[PQV Widget] Starting voice recording");
                const stream =
                ...
                console.log("[PQV Widget] Stopping voice recording");
                mediaRecorder.stop();
                voiceBtn.classList.remove("recording");
              }
            } catch (error) {
              safeLogError("[PQV Widget] Error accessing microphone", error);
            }
          });
        }
        ...
        async function sendMessage() {
          ...
          try {
            ...
            safeLogError("[PQV Widget] Failed to send message", error);
            hideTypingIndicator();
            addMessage({
              senderType: "ai",
              content:
        ...
        async function sendVoiceMessage(audioBlob) {
          ...
          try {
            ...
            safeLogError(
              "[PQV Widget] Failed to send voice message",
              error
            );
            hideTypingIndicator();
          }
        ...
        async function loadConversation() {
          try {
            const response = await fetch(
              `${API_BASE}/api/widget/conversation?widgetId=${encodeURIComponent(
                WIDGET_ID
              )}&visitorId=${encodeURIComponent(VISITOR_ID)}`
            );
            const data = await response.json();
            ...
            safeLogError(
              "[PQV Widget] Failed to load conversation",
              error
            );
          }
        }
        ...
        async function trackEvent(eventType) {
          try {
            ...
            safeLogError("[PQV Widget] Failed to track event", error);
          }
        }
      }
      ...
      safeLogError("[PQV Widget] initWidget fatal error", err);
    }
  }

  if (typeof window === "undefined") {
    // SSR safety – do nothing
    console.log("[PQV Widget] window undefined (SSR), skipping init");
    return;
  }
